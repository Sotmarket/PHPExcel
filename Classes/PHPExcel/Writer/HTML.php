...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	 ...
	 * @var boolean
	 */
	protected $_spansAreCalculated	= false;
	 ...
	 * @var array
	 */
	protected $_isSpannedCell	= array();
	 ...
	 * @return string
	 */
	protected function _mapVAlign($vAlign) {
		switch ($vAlign) {
	 ...
	 * @return string|false
	 */
	protected function _mapHAlign($hAlign) {
		switch ($hAlign) {
	 ...
	 * @return	string
	 */
	protected function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
        ...
        if ("Untitled Spreadsheet" == $properties->getTitle()){
            $title = $this->_phpExcel->getActiveSheet()->getTitle();
        }
		...
		if ($properties->getTitle() > '')
			$html .= '	  <meta name="title" content="' . htmlspecialchars($title) . '" />' . PHP_EOL;
	...
	protected function _extendRowsForChartsAndImages(PHPExcel_Worksheet $pSheet, $row) {
		$rowMax = $row;
		$colMax = 'A';
	 ...
	 * @throws	PHPExcel_Writer_Exception
	 */
	protected function _writeImageInCell(PHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
...
//                        $imageData . '" border="0" />';
                    ...
                    $html .= '<img style="position: ' . $position . '; z-index: 1; left: ' . $drawing->getOffsetX(
                        ) . 'px; top: ' . $drawing->getOffsetY() . 'px; width: ' . $drawing->getWidth(
                        ) . 'px; height: ' . $drawing->getHeight(
                        ) . 'px;" src="' . $imageData . '" border="0" />' . PHP_EOL;
					$html .= '</div>';
				}
			}
	 ...
	 * @throws	PHPExcel_Writer_Exception
	 */
	protected function _writeChartInCell(PHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @return	array
	 */
	protected function _createCSSStyle(PHPExcel_Style $pStyle) {
		// Construct CSS
		$css = '';
	 ...
	 * @return	array
	 */
	protected function _createCSSStyleAlignment(PHPExcel_Style_Alignment $pStyle) {
		// Construct CSS
		$css = array();
	 ...
	 * @return	array
	 */
	protected function _createCSSStyleBorders(PHPExcel_Style_Borders $pStyle) {
		// Construct CSS
		$css = array();
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	 ...
	 * @return	array
	 */
	protected function _createCSSStyleFill(PHPExcel_Style_Fill $pStyle) {
		// Construct HTML
		$css = array();
	 ...
	 * @throws	PHPExcel_Writer_Exception
	 */
	protected function _generateTableFooter() {
		// Construct HTML
		$html = '';
		$html .= '	</table>' . PHP_EOL;
...
//					}
	 ...
	 */
	protected function _calculateSpans()
	{
	...
	protected function _setMargins(PHPExcel_Worksheet $pSheet) {
		$htmlPage = '@page { ';
		$htmlBody = 'body { ';
